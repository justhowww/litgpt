#!/bin/bash
# MRT-primary learned stopping, desaturated reward, GT dropped from candidate pool.
#
# One variable changed vs. stage2_fim_psm_mrt_primary_learned_stop_desat_1k_8k:
#   MRT_INCLUDE_GROUND_TRUTH: 1 -> 0
# All 8 candidates are model-sampled. Tests whether GT-in-pool was anchoring
# (upper-bound reference) or hijacking the gradient (CE-like reranking)
# under the non-saturating scaled_mse reward.
...
# The one variable that differs from the desat learned-stop primary.
export MRT_INCLUDE_GROUND_TRUTH=0

# Same reward calibration as the desat primary so risk curves are comparable.
# r_i = MSE_i / 0.035, MRT_MSE_WEIGHT = 1/0.035 = 28.5714, decode failure = 5.0.
...
export MODEL_NAME=${MODEL_NAME:-byte-stage2-fim-psm-mrt-primary-learned-stop-desat-nogt-1k-8k}
export OUT_DIR=${OUT_DIR:-"${STAGED_CORPUS}/runs/byte-stage2-fim-psm-mrt-primary-learned-stop-desat-nogt-1k-8k"}
