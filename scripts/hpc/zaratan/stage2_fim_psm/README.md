...
cross-entropy; the `stop_*` metrics are the real signal. During EOS runs the
probe also allows generation up to 2x the oracle length so over-runs are
visible, not just early stops.
