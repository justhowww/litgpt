#!/bin/bash
# Run this wrapper on a Zaratan login node. It submits the SCALED Stage 0
# multi-frame AR (H0-XL) training against the compute-node-visible staged corpus.
...
OUT_DIR=${OUT_DIR:-"${STAGED_CORPUS}/runs/byte-stage0-ar-multiframe-xl"}
...
echo "Submitted Stage 0 multi-frame AR-XL training job ${job_id}"
