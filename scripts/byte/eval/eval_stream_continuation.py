...
def evaluate_checkpoint(
    ...
    desync_categories: Counter = Counter()  # its category (coarse; for density)
    desync_reasons: Counter = Counter()  # parser exception class -> failure mechanism
    exposure_bytes: Counter = Counter()  # survived generated bytes per category
    ...
    summary = {
        ...
        # Exposure-normalized desync density per category: desyncs per 1000 survived
        # bytes of that category. Removes the base-rate confound (residual is most
        # bytes) so brittleness (e.g. residual/coeff_token) is comparable across fields.
        "desync_density_per_kb": {
            cat: round(1000.0 * n / exposure_bytes[cat], 3)
            for cat, n in desync_categories.most_common()
            if exposure_bytes.get(cat)
        },
        "desync_category_hist": dict(desync_categories.most_common()),
        "exposure_bytes": dict(exposure_bytes.most_common()),
...
def generate_continuation(
    ...
    ``prefix_bytes`` = concat of nals[start_nal:end]."""
    raw = model.module if hasattr(model, "module") else model
    raw.eval()
