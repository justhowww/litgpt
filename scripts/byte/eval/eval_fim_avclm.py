...
def teacher_forced_span_metrics(
    ...
    exact_tail = bool(
        int((pred == supervised_labels).sum()) == int(supervised_labels.numel())
    )
...
def first_mismatch_diagnostic(
    ...
    tf_argmax = (
        tf_argmax_ids[mismatch_pos] if mismatch_pos < len(tf_argmax_ids) else None
    )
    return {
        ...
        "tf_correct_at_first_mismatch": (
            bool(tf_argmax == gt_byte)
            if gt_byte is not None and tf_argmax is not None
            else None
        ),
        "free_matches_tf_argmax_at_first_mismatch": (
            bool(tf_argmax == gen_byte)
            if gen_byte is not None and tf_argmax is not None
            else None
        ),
    }
...
def summarize(details: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    out = {
        ...
        "length_match_rate": mean(
            1.0 if r["length_delta"] == 0 else 0.0 for r in details
        ),
    ...
    mask_rows = [r for r in details if r.get("mask_enabled")]
    if mask_rows:
        mask_calls = sum(r["mask_calls"] for r in mask_rows)
        strict_calls = sum(r["mask_strict_calls"] for r in mask_rows)
        out.update(
            {
                "mask_calls_total": mask_calls,
                "mask_strict_calls_total": strict_calls,
                "mask_permissive_calls_total": sum(
                    r["mask_permissive_calls"] for r in mask_rows
                ),
                "mask_strict_call_rate": strict_calls / max(mask_calls, 1),
                "mask_argmax_rejected_total": sum(
                    r["mask_argmax_rejected"] for r in mask_rows
                ),
                "mask_eos_checks_total": sum(r["mask_eos_checks"] for r in mask_rows),
                "mask_eos_blocked_total": sum(r["mask_eos_blocked"] for r in mask_rows),
            }
        )
    return out
    ...
    for checkpoint_dir in args.checkpoint_dirs:
        ...
        for stop_mode in args.stop_modes:
            ...
            for sample_id, sample in enumerate(samples):
                ...
                row.update(
                    first_mismatch_diagnostic(
                        result.data, sample.target_bytes, tf_metrics
                    )
                )
                ...
                if args.decode:
                    tmp_gt = (
                        args.out_dir
                        / f".tmp_{ckpt_name}_{stop_mode}_{sample_id}_gt.h264"
                    )
                    tmp_model = (
                        args.out_dir
                        / f".tmp_{ckpt_name}_{stop_mode}_{sample_id}_model.h264"
                    )
                    gt_ok, gt_err = decode_ok(
                        gt_stream, args.ffmpeg_binary, args.timeout_sec, tmp_gt
                    )
