...
def build_free_run_sample(
    base_dataset,
    idx: int,
    config: FreeRunEvalConfig,
) -> FreeRunSample | None:
    """Build one SPS-anchored AR continuation context.

    Index selection deliberately lives outside this function.  Given one window
    index, it either returns the exact :class:`FreeRunSample` used by evaluation
    and GRPO or ``None`` when that window cannot support the requested frame
    geometry.  Keeping this builder deterministic makes an online GRPO context
    reproducible from only ``(dataset index, config)``.
    """
    ...
    win = base_dataset.samples[idx]
    if win.num_frames < needed:
        return None
    nals = base_dataset.nal_index[str(win.h264_path)]
    ...
    # ar_item, not base_dataset[idx]: this context is AR-only and needs labels
    # to be the window's raw bytes. Under p_fim > 0 __getitem__ may return FIM.
    item = base_dataset.ar_item(idx)
    window_labels = item["labels"]
    if base_dataset.use_eos:
...
def prepare_free_run_samples(
    ...
    for idx in indices:
        sample = build_free_run_sample(base, idx, config)
