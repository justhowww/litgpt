...
def free_run_rollout(
    ...
    that keep the stream decodable (Annex-B escaping + CAVLC grammar). Requires
    ``prefix_bytes`` to seed the mask state (SPS/PPS + the open NAL at the prefix end).
    An all-illegal mask means the grammar boxed us in -> stop like any other desync.
    """
    if max_gen <= 0:
        return b"", 0
    mask_state = None
