...
def free_run_rollout(
    ...
    accepted by the supported H.264 slice-data automaton plus Annex-B framing. Requires
    ``prefix_bytes`` to seed the mask state. ``mask_residual_only`` reproduces the old
    residual-only, fail-open mask for a controlled ablation. An all-illegal full mask
    means the grammar boxed us in -> stop like any other desync.
