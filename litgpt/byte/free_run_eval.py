...
def _desync_region(nal) -> str | None:
    """Syntax element being read when the NAL desynced: the last recorded span at or
    before ``desync_bit`` (the break is in the element right after it). Returns the
    raw span name (caller normalizes array indices), or None if unavailable.
    """
    if nal is None or not nal.spans:
        return None
    region = nal.spans[-1]
    dbit = nal.desync_bit
    if dbit is not None:
...
def _survival_and_validity(
    ...
    survival_bytes = generated bytes before the first CAVLC desync at or after the
    prefix boundary. A desync counts even when it lands in a NAL that *began* in the
    prefix -- that is the case where the model emitted no leading start code and its
    bytes were absorbed into the trailing prefix slice (Annex-B NALs run to the next
    start code). If nothing desyncs but the model never emitted a generated start
    code, it never began a valid frame, so survival is 0 (not the full budget). Only
    when a real generated frame was started and the whole budget parses clean is
    survival = len(generated).

    desync_region = the H.264 syntax element the stream was reading at the first
    desync (e.g. ``coeff_token``, ``run_before``), or None if it parsed clean.
    """
    ...
    if first_desync is not None:
        ...
        survival = len(generated)
    return survival, min(valid_cont, cont_frames), start_codes, _desync_region(desync_nal)
...
def run_free_run_eval(
    ...
    try:
        for sample in samples:
            ...
            survival, valid_cont, start_codes, _region = _survival_and_validity(
