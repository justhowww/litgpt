...
class GPT(nn.Module):
    ...
    def _megabyte_local_transformer(self, local_x: torch.Tensor) -> torch.Tensor:
        """Run the shared local Transformer over independent byte patches."""
        local_length = local_x.size(1)
        local_cos = self.megabyte_local_cos[:local_length].unsqueeze(0)
        local_sin = self.megabyte_local_sin[:local_length].unsqueeze(0)
        ...
        return self.megabyte_local.ln_f(local_x)

    def megabyte_local_next_logits(
        self,
        global_output: torch.Tensor,
        previous_bytes: torch.Tensor,
    ) -> torch.Tensor:
        """Predict the next byte inside a patch without observing future bytes.

        Args:
            global_output: Global Transformer output for the patch being
                generated, shape ``(B, n_embd)``.
            previous_bytes: Already generated bytes in this patch, shape
                ``(B, p)`` where ``0 <= p < byte_patch_size``.
        """
        patch_size = self.config.byte_patch_size
        if patch_size <= 1:
            raise ValueError("megabyte_local_next_logits requires byte patches")
        if global_output.dim() != 2 or global_output.size(-1) != self.config.n_embd:
            raise ValueError(
                "global_output must have shape "
                f"(B,{self.config.n_embd}), got {tuple(global_output.shape)}"
            )
        if previous_bytes.dim() != 2 or previous_bytes.size(0) != global_output.size(0):
            raise ValueError(
                "previous_bytes must have shape (B,p) with the same batch size"
            )
        local_length = previous_bytes.size(1) + 1
        if local_length > patch_size:
            raise ValueError(
                f"cannot generate byte {local_length} in a size-{patch_size} patch"
            )

        B = global_output.size(0)
        global_byte_dim = self.config.n_embd // patch_size
        global_condition = self.megabyte_global_to_local(
            global_output.reshape(B, patch_size, global_byte_dim)
        )[:, :local_length]
        local_pad = self.megabyte_local.pad_wte.weight[0].view(1, 1, -1).expand(
            B, 1, -1
        )
        if previous_bytes.numel():
            local_previous = self.megabyte_local.wte(previous_bytes)
            local_previous = torch.cat((local_pad, local_previous), dim=1)
        else:
            local_previous = local_pad
        local_positions = self.megabyte_local.pos_wte.weight[:local_length].unsqueeze(0)
        local_x = global_condition + local_previous + local_positions
        local_x = self._megabyte_local_transformer(local_x)
        logits = F.linear(local_x[:, -1], self.megabyte_local.wte.weight)
        if self.config.final_logit_softcapping is not None:
            logits = do_softcapping(
                logits, thresh=self.config.final_logit_softcapping
            )
        return logits
    ...
    def forward(
        ...
        x = self._global_transformer_forward(
            idx,
            input_pos=input_pos,
    ...
    def megabyte_global_forward(
        ...
        """Run only the cached global patch Transformer during generation."""
        patch_size = self.config.byte_patch_size
        if patch_size <= 1:
            raise ValueError("megabyte_global_forward requires byte patches")
        if idx.dim() != 3 or idx.size(-1) != patch_size:
            raise ValueError(
                f"expected idx shape (B,T,{patch_size}), got {tuple(idx.shape)}"
            )
        return self._global_transformer_forward(
            idx,
            input_pos=input_pos,
            input_pos_maxp1=input_pos_maxp1,
            region_ids=region_ids,
    ...
    def _global_transformer_forward(
        self,
        idx: torch.Tensor,
        *,
        ...
        """Shared global Transformer path for training and cached generation."""
        patch_size = self.config.byte_patch_size
        T = idx.size(1)
